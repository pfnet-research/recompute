...
long long eval(const DAG &dag, SCHEDULE sch){
	...
	for(ORDER ord: sch){
		...
		/*for(int i = 0 ; i < 30 ; i ++){
			if(competed_forward[i])fprintf(stderr,"1");
			else fprintf(stderr,"0");
		}
		fprintf(stderr,"\n");
		for(int i = 0 ; i < 30 ; i ++){
			if(competed_backward[i])fprintf(stderr,"1");
			else fprintf(stderr,"0");
		}
		fprintf(stderr,"\n");
		fprintf(stderr,"=================\n");*/
...
bool comp(std::pair<int,subgraph> x, std::pair<int,subgraph> y){
...
int main(int argc, char *argv[]){
	...
	std::vector<std::pair<int,subgraph>> cuts_;
	subgraph start;
	cuts_.push_back(std::pair<int,subgraph>(0,start));
	for(int i = 0 ; i < forward.N ; i ++){
		subgraph s;
		std::queue<int> que;
		s[i] = true;
		...
		cuts_.push_back(std::pair<int,subgraph>(forward.m_G(s),s));
	}
	std::sort(cuts_.begin(),cuts_.end(),comp);
	for(int i = 0 ; i < cuts_.size() ; i ++){
		forward.cuts.push_back(cuts_[i].sc);
		forward.cut_id[cuts_[i].sc] = i;
	}
	...
	/*for(std::pair<int,int> p: dp[end_id]){
		if(p.fr > loc.fr)loc = p;
	}*/
	if(MAX_BATCHSIZE){
		loc = *dp[end_id].begin();
